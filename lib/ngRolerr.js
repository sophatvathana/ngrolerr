...
Object.defineProperties(this, {
       ...
       expire: {
        ...
        header: {
          get: function() { return config.header; },
          set: function(value) {return config.header = value; }
        }
   ...
   this.$get = ['service', 'rolerr', 'authorizer', function(service, rolerr, authorizer){
      ...
      $rolerr.customLogin = function(token){
        return service.customLogin(token);
      };
    ...
    return {
      ...
      $http.get(config.userRole).success(function(data){
       ...
       customLogin: function(token){
        var deferred = $q.defer();
        return $http.get(config.customUrl+"?token="+token).success(function(data){
            deferred.resolve(data);
            rolerr.authenticate(token);
         }).error(function(err){deferred.reject(err); });
       },
    ...
    return {
      ...
      authenticate: function(identity) {
        ...
        var expireDate = new Date();
             expireDate.setDate(expireDate.getDate() + config.expire);
        if (identity)
          ...
          if (!rolerr.isAuthenticated()){
            ...
            if($rootScope.toState.data!==undefined){
             if ($rootScope.toState.data.roles!==undefined)
              if($rootScope.toState.data.roles.length > 0 && !shareservice.checkRolesSource(data, $rootScope.toState.data.roles)) {
