...
* @Last Modified time: 2015-09-16 01:03:12
...
Object.defineProperties(this, {
       ...
       storageName: {
          get: function() { return config.storageName; },
          set: function(value) { config.storageName = value; }
       }
     ...
     var windowStorage = function(type) {
      ...
      $storage.setItem = function(key,value){
        if(!isCookies)
          ...
          return isCookies ? $rolerrCookies.put(key, value) : undefined;
      };
  ...
  'ngRolerrConfig', '$http', function($rolerrCookies, rolerr, $q, config, $http){
    ...
    return {
      ...
      $http.get(config.userRole).success(function(data){
       ...
       logout: function(){
          $rolerrCookies.remove(config.storageName);
         return $q.when();
       },
  ...
  function(config, $q, $window, $timeout, $storage) {
    ...
    return {
      ...
      isAuthenticated: function() {
        ...
        var _token = $storage.getItem(config.storageName);
          if (_token) {
      ...
      authenticate: function(identity) {
        ...
        if (identity)
           $storage.setItem(config.storageName, identity,{'expires': expireDate});
        else $storage.removeItem(config.storageName);
      },
      ...
      identity: function(force) {
        ...
        var self = this;
         $timeout(function() {
          _token =  $storage.getItem(config.storageName);
        ...
        request: function(request) {
         // if (request.skipAuthorization) {
          ...
          if (rolerr.isAuthenticated() && config.modeInterceptor) {
             var token =  $storage.getItem(config.storageName);

                token =config.header && config.tokenType ? config.tokenType + ' ' + token:
                    $storage.getItem(config.storageName);
